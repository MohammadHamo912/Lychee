...
const OrderManagement = ({ role = 'shopowner', storeId: propStoreId }) => {
    ...
    useEffect(() => {
        ...
        const getOrders = async () => {
            ...
            else if (role === 'customer') params.userId = user.userId;
        ...
        getOrders();
    }, [filterStatus, searchQuery, startDate, endDate, role, user, effectiveStoreId]);
    ...
    const handleCardClick = async (order) => {
        ...
        try {
            ...
            if (role === 'shopowner') {
                ...
                items = await fetchOrderItemDetailsByStore(effectiveStoreId, order.orderId);
            } else {
    ...
    const handleStatusUpdate = async () => {
        try {
            ...
            const updated = orders.map((o) =>
                o.orderId === selectedOrder.orderId ? { ...o, status: selectedOrder.status } : o
            );
            setOrders(updated);
                ...
                <div className="order-popup" onClick={closeModal}>
                    <div className="order-popup-content" onClick={(e) => e.stopPropagation()}>
                        ...
                        {role === 'admin' ? (
                            <div>
                                <label>Status:</label>
                                <select value={selectedOrder.status} onChange={handleStatusChange}>
                                    <option value="Confirmed">Confirmed</option>
                                    <option value="Shipping">Shipping</option>
                                    <option value="Delivered">Delivered</option>
                                </select>
